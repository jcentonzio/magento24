...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
        ...
        $select = $connection->select()->from(['product' => $productCollection->getTable('catalog_product_entity')], ['entity_pk_value' => 'entity_id'])
                ->joinLeft(
                    ['rt' => $connection->select()
                        ->from(
                            ['rova' => $productCollection->getTable('rating_option_vote_aggregated')],
    ...
    public function isHidePriceFilter($filter)
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $httpContext = $objectManager->get('Magento\Framework\App\Http\Context');
        $isLoggedIn = $httpContext->getValue(\Magento\Customer\Model\Context::CONTEXT_AUTH);

        if (get_class($filter) == "Codazon\AjaxLayeredNavPro\Model\Layer\Filter\Price") {
            $filterIsPrice = true;
        } else {
            $filterIsPrice = false;
        }

        $optionHidePriceFilter = $this->getConfig('codazon_ajaxlayerednavpro/general/hide_price_filter');

        if ($optionHidePriceFilter == '1' && $filterIsPrice && !$isLoggedIn) {
            return "hide";
        } else {
            return "show";
        }
