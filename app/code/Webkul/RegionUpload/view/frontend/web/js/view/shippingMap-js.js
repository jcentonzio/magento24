...
define([
  ...
  ], function ($, Component, ko, $t, alert,mapData) {
    'use strict';
    var countryId = '';
    var countryName = '';
    var postalCode = '';
    ...
    var mapDataValue = mapData.getMapData();
     return Component.extend({
        // defaults: {
        //     template: 'Webkul_RegionUpload/form/element/elements'
        // },
        initialize: function () {
            this._super();
        ...
        initCustomEvents: function () {
            var self = this;
            $(document).find("#opc-new-shipping-address div[name = 'shippingAddress.telephone']").append($(".mapContainer"));
            if (!$(document).find("#opc-new-shipping-address").length) {
                     $(document).find("div[name = 'shippingAddress.telephone']").append($(".mapContainer"));
                     $(".mapContainer").css({'height': '13%',
                                              'width': '500px',
                                              'margin-top': '10px',
                                              'border':'1px solid rgb(5, 162, 253)'});
                      if (mapDataValue['status'] != '0') {
                        if ($(document).find(".mapContainer").length) {
                      $(document).find("#opc-shipping_method").css({'margin-top': '278px'});
                      }
                    }
            }
